...
function ProductAmenities() {
  return (
    <section className="mt-10 space-y-5">
      <h2 className="text-lg font-medium text-dark-primary">امکانات رفاهی</h2>
      ...
      <ul className="grid grid-cols-1 mobile-landscape:grid-cols-2 tablet-portrait:grid-cols-3 tablet-landscape:grid-cols-4 gap-4">
        {amenities.map((item) => {
          ...
          return (
            <li
              key={item.title}
              className="flex items-center gap-3 rounded-xl border border-dark-border p-4 transition-all duration-300 hover:-translate-y-1 hover:border-dark-primary/40"
            >
              <Icon className="text-2xl text-dark-primary" />
